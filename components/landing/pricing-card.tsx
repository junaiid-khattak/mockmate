  ...
  return (
    <Card
      className={cn(
        ...
        plan.popular ? "border-[#7c5cfc]/40 bg-gradient-to-b from-[rgba(124,92,252,0.06)] to-white" : "",
      )}
    >
      {plan.popular ? (
        <Badge className="absolute right-4 top-4 bg-[#7c5cfc] text-white shadow-sm">Best experience</Badge>
      ) : null}
      <CardHeader className="space-y-3">
        ...
        <div className="text-3xl font-semibold text-slate-900 dark:text-white">
          {plan.price}
          {plan.priceLabel && (
            <span className="text-base font-normal text-slate-500">{plan.priceLabel}</span>
          )}
        </div>
        {plan.note && (
        ...
        {plan.premiumCallout && (
          <div className="rounded-lg border border-[rgba(124,92,252,0.18)] bg-[rgba(124,92,252,0.06)] px-3 py-2">
      ...
      <CardContent className="mt-auto space-y-4">
        ...
        <Button
          ...
          className={cn(
            "w-full",
            plan.popular ? "bg-[#7c5cfc] hover:bg-[#6a4be0]" : "",
          )}
          variant={plan.highlighted || plan.popular ? "default" : "secondary"}
          onClick={onCta}
        >
          {plan.cta}
        </Button>
